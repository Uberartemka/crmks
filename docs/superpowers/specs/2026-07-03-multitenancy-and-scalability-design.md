...
текущую компанию без ручной перенумерации. После миграции дефолт убирается.
...
После этого `SELECT * FROM clients` **автоматически** возвращает только строки
текущего тенанта — программисту не надо писать фильтр в каждом запросе.
...
2. **Мультитенантность: таблица `tenants`, `tenant_id` по таблицам, RLS,
   middleware.** Фундамент, без него нельзя принимать вторую компанию.
