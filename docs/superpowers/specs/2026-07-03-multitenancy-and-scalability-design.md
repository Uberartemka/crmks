...
- **`catalog_pool` намеренно без tenant-контекста.** Каталог не тенантный и
  читается вторым пулом без RLS вообще, поэтому `SET LOCAL app.tenant_id` там
  не нужен и не вызывается. В lifespan это надо отметить явным комментарием
  («catalog_pool не участвует в tenant-контексте намеренно — каталог общий»),
  иначе через полгода кто-то по аналогии попробует прикрутить туда `SET LOCAL`
  и запутается или сломает чтение.
- **JWT как источник `tenant_id` — устаревание токенов.** Middleware доверяет
  `tenant_id` из JWT. Значит, при смене пароля / логауте / переносе юзера
  между тенантами старые токены должны инвалидироваться — иначе RLS будет
  работать корректно, но **по устаревшему `tenant_id`** (юзер формально уже
  в компании Б, но видит данные компании А, пока не истечёт TTL токена).
  Инвалидация — на уровне `token_store` (Redis, уже есть): запись
  `jti`/`token_version` юзера и проверка в middleware. Это отдельная
  задача в плане, не архитектурный риск.
