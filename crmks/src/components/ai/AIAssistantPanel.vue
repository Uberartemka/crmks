...
import { Send, Sparkles, Trash2, ChevronLeft, ChevronRight, X } from 'lucide-vue-next'
...
watch(() => ai.messages.length, (newLength) => {
  if (newLength > 0) {
...
<template>
  <aside 
    class="flex flex-col h-full bg-slate-50 border-l border-slate-200 shrink-0 transition-all duration-300 ease-in-out"
    :class="isExpanded ? 'w-[550px]' : 'w-[360px]'"
  >
    <header class="flex items-center justify-between px-3 py-2 border-b border-slate-200 bg-white">
      <div class="flex items-center gap-2">
        <button 
          ...
          :title="isExpanded ? 'Сжать панель' : 'Расширить панель'"
        >
      ...
      <div class="flex items-center gap-1">
        ...
        <button class="btn-ghost !p-1" @click="close" title="Закрыть">
          <X :size="14" />
        </button>
      </div>
    </header>
