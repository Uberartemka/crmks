"""
Token store — always uses PostgreSQL (no Redis dependency).

On Railway there is no persistent Redis, so we store tokens in the
`auth_tokens` table in PostgreSQL.  This works everywhere.
"""
...
TOKEN_TTL_SECONDS = int(__import__("os").getenv("TOKEN_TTL_SECONDS", "86400"))
...
def _ensure_auth_tokens_table() -> None:
    ...
    try:
        ...
        if _use_pg:
            ...
            """)
            # Migrate old timestamp without timezone → with timezone
            cursor.execute("""
                DO $$
                BEGIN
                    IF EXISTS (
...
async def init_token_store() -> None:
    """Initialize token store — ensures the auth_tokens table exists."""
    _ensure_auth_tokens_table()
    logger.info("[token_store] Using PostgreSQL for token storage.")


async def close_token_store() -> None:
    """No-op — nothing to close for PG-based store."""
    pass
