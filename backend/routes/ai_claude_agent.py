...
# Короткий system-prompt для эндпоинтов, где нужен только структурированный JSON
# (вместо 18-строчного AGENT_SYSTEM_PROMPT). Экономит ~150 токенов на запрос.
JSON_SYSTEM_PROMPT = "Верни ТОЛЬКО валидный JSON. Без markdown, без пояснений, без вступлений."


async def call_claude(prompt: str, system: str = AGENT_SYSTEM_PROMPT) -> str:
    """
    Отправляет запрос в LLM-провайдер по каскаду:
    1. GLM (BigModel/Zhipu) — primary. Использует вашу подписку GLM.
    2. Anthropic Claude — фолбэк, если задан ANTHROPIC_AUTH_TOKEN.
    3. Kimi через Cloudflare — последний фолбэк.

    Первая успешная ветка возвращает ответ; при ошибке переходим к следующей.
    """
    import httpx
    ...
    # --- 1. GLM (BigModel) — PRIMARY ---
    glm_key = os.getenv("GLM_API_KEY")
    if glm_key:
        glm_model = os.getenv("GLM_MODEL", "glm-4.5-flash")
        try:
            logger_local.info(f"[call_claude] Отправка запроса в GLM ({glm_model})...")
            async with httpx.AsyncClient() as client:
                ...
                logger_local.info("[call_claude] Успешный ответ от GLM.")
                return result
        except Exception as e:
            logger_local.warning(
                f"[call_claude] Ошибка при запросе к GLM: {e}. Пробуем фолбэк (Anthropic/Kimi)..."
            )
    ...
    raise HTTPException(
        500, "Ошибка ИИ: не настроены ключи ни для GLM (GLM_API_KEY), ни для Anthropic (ANTHROPIC_AUTH_TOKEN), ни для Cloudflare (CF_API_TOKEN)."
    )
...
Предложи ровно 3 варианта следующего действия (от срочного к отложенному).
Верни ТОЛЬКО JSON вида:
{{"options": [{{"id":1,"label":"...","action":"create_kp|create_task|followup","priority":"urgent|high|normal|low","due_hours":число}}], "recommended": id}}

Выбери recommended исходя из контекста звонка."""
