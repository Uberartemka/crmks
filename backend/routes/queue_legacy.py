...
@router.post("/api/queue_legacy/add")
...
@router.get("/api/queue_legacy/status/{task_id}")
...
@router.get("/api/queue_legacy/list", dependencies=[Depends(verify_b2b_token)])
def list_tasks():
    manager = get_queue_manager()
    return manager.list_tasks(limit=50)


@router.get("/api/queue_legacy/stats", dependencies=[Depends(verify_b2b_token)])
def get_stats():
    manager = get_queue_manager()
    return manager.get_queue_stats()


@router.post("/api/queue_legacy/retry/{task_id}", dependencies=[Depends(verify_b2b_token)])
...
@router.post("/api/webhooks_legacy/bitrix", dependencies=[Depends(verify_b2b_token)])
def bitrix_webhook(payload: BitrixWebhookInput):
    ...
    task_payload = {"event_type": payload.event, "deal_id": deal_id, "raw_data": payload.data}
...
@router.post("/api/webhooks_legacy/bitrix/telephony", dependencies=[Depends(verify_b2b_token)])
def bitrix_telephony_webhook(payload: BitrixTelephonyInput):
    ...
    crm_entity_type = call_data.get("CRM_ENTITY_TYPE")
            ...
            cursor.execute(
                q(
                    ...
                    INSERT INTO call_logs
                        (user_id, lead_id, client_name, to_number, direction,
                         call_date, status, duration, recording_url, bitrix_call_id,
...
@router.post("/api/webhooks_legacy/1c", dependencies=[Depends(verify_b2b_token)])
def one_c_webhook(payload: OneCWebhookInput):
    ...
    task_payload = {"sku": payload.sku, "new_stock": payload.new_stock, "new_price": payload.new_price}
