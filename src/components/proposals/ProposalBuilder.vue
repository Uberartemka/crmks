...
const items = computed(() => currentProposal.value?.items || [])
...
const totalAfterDiscount = computed(() =>
  items.value.reduce((s, it) => s + it.qty * it.price_final, 0)
)
...
async function applyGlobalDiscount() {
  if (!currentProposalId.value) { alert('Сначала создайте КП'); return }
...
function downloadPdf() {
  if (!currentProposalId.value) return
  window.open(`${API_BASE}/api/proposals/${currentProposalId.value}/pdf`, '_blank')
}
  ...
  <div class="p-6 space-y-6">
    ...
    <div class="card p-5 grid lg:grid-cols-4 gap-4 items-start">
      ...
      <div class="flex gap-2 pt-6">
        ...
        <button @click="sendEmail" :disabled="!currentProposalId" class="flex-1 btn-primary text-xs disabled:opacity-40 flex items-center justify-center gap-1">
    ...
    <!-- Secondary actions -->
    <div class="flex gap-2">
      <button @click="openPublicLink" :disabled="!currentProposalId" class="btn-ghost text-xs disabled:opacity-40 flex items-center gap-1">
      ...
      <button @click="downloadPdf" :disabled="!currentProposalId" class="btn-ghost text-xs disabled:opacity-40 flex items-center gap-1">
      ...
      <div class="lg:col-span-2 card p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <button @click="printPage" class="text-[10px] text-brand-700 hover:underline flex items-center gap-1 font-semibold">
            <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2M6 14h12v8H6z"/></svg>
            Печать
          </button>
        </div>
          ...
          <table class="w-full text-[11px] text-left">
            ...
            <tbody class="divide-y divide-neutral-100">
              ...
              <tr v-for="it in items" :key="it.id" class="hover:bg-slate-50 transition">
                ...
                <td class="px-2 py-2 text-right font-bold text-neutral-800">{{ Math.round(it.price_final).toLocaleString('ru-RU') }} ₽</td>
                <td class="px-2 py-2 text-center">
      ...
      <div class="space-y-2">
        ...
        <table class="w-full text-xs border-collapse">
          <thead class="bg-brand-700 text-white">
            <tr><th class="px-2 py-1 text-left">Артикул</th><th class="px-2 py-1 text-center">Кол</th><th class="px-2 py-1 text-right">Цена</th><th class="px-2 py-1 text-right">Сумма</th></tr>
          </thead>
          <tbody>
            <tr v-for="it in items" :key="it.id" class="border-b border-neutral-200">
              ...
              <td class="px-2 py-1 text-right">{{ Math.round(it.price_final).toLocaleString('ru-RU') }} ₽</td>
              <td class="px-2 py-1 text-right">{{ Math.round(it.price_final * it.qty).toLocaleString('ru-RU') }} ₽</td>
            </tr>
          </tbody>
        </table>
